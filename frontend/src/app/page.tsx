...
import { MessageCircle, Brain, TrendingUp, Volume2, Sparkles, ArrowRight, CheckCircle, ChevronDown, Github, Menu, X } from "lucide-react"
...
export default function LandingPage() {
	...
	useEffect(() => {
		...
		setDynamicHeight(); // Set initially
	...
	// Close mobile menu when screen size changes (existing logic)
	useEffect(() => {
		const handleResize = () => {
	...
	return (
		...
		>
			...
			<nav className="sticky top-0 w-full z-50 bg-[#1e1e2e]/80 backdrop-blur-sm border-b border-[#313244]">
				<div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-4 flex justify-between items-center">
					...
					<a href="#" className="flex items-center space-x-3 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
						...
						<span className="text-xl font-semibold text-[#f9e2af] group-hover:text-[#b4befe] transition-colors">Whisper</span>
					</a>
				...
				{isMobileMenuOpen && (
					<div className="md:hidden bg-[#1e1e2e] border-t border-[#313244] py-4">
						<div className="flex flex-col items-center space-y-4">
							<a href="#features" className="text-[#bac2de] hover:text-[#f9e2af] transition-colors text-base font-medium" onClick={() => setIsMobileMenuOpen(false)}>
								Features
							</a>
							<a href="#how-it-works" className="text-[#bac2de] hover:text-[#f9e2af] transition-colors text-base font-medium" onClick={() => setIsMobileMenuOpen(false)}>
								How it works
							</a>
							<a href="/chat" className="bg-[#cba6f7] text-[#1e1e2e] px-6 py-2 rounded-lg text-base font-medium hover:bg-[#b4befe] transition-colors" onClick={() => setIsMobileMenuOpen(false)}>
								Get started
							</a>
						</div>
					</div>
			...
			<section className="min-h-[100dvh] flex flex-col snap-start snap-always bg-[#1e1e2e]">
				<div className="flex-grow flex flex-col justify-center items-center px-4 sm:px-6 md:px-8 pt-32 pb-24">
					<div className="max-w-3xl w-full mx-auto text-center space-y-8 relative z-10">
						...
						{/* Floating elements */}
						<div className="absolute top-8 left-4 opacity-20 animate-bounce hidden sm:block" style={{ animationDelay: '0.5s' }}>
							<Brain className="h-8 w-8 text-[#cba6f7]" />
						</div>
						<div className="absolute bottom-8 right-4 opacity-20 animate-bounce hidden sm:block" style={{ animationDelay: '1.5s' }}>
							<MessageCircle className="h-6 w-6 text-[#89b4fa]" />
						</div>
						<div className="absolute top-24 right-8 opacity-20 animate-bounce hidden sm:block" style={{ animationDelay: '2.5s' }}>
			...
			{/* Quiz Mode Section */}
			<section id="quiz-mode" className="h-screen-dynamic flex flex-col snap-start snap-always">
			...
			<section id="learning-tracker" className="h-screen-dynamic flex flex-col snap-start snap-always bg-[#181825]">
				<div className="flex-grow overflow-y-auto pt-54 pb-64 px-6 sm:px-4 md:px-6 lg:px-8 flex items-center justify-center">
					<div className="container mx-auto">
						<div className="grid lg:grid-cols-2 gap-12 items-center">
							<div>
			...
			{/* Voice Section */}
			<section id="voice" className="h-screen-dynamic flex flex-col snap-start snap-always">
			...
			<section id="how-it-works" className="h-screen-dynamic flex flex-col snap-start snap-always bg-[#181825]">
				<div className="flex-grow overflow-y-auto pt-54 pb-64 px-6 sm:px-4 md:px-6 lg:px-8 flex items-center justify-center">
