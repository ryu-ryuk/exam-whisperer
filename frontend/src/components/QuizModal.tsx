...
// If QuizModal is defined in this file, update its definition:
function QuizModal({ open, onClose, defaultTopic, onQuizComplete }: QuizModalProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onQuizComplete({ correct: answer.trim().toLowerCase() === "paris" ? 1 : 0, total: 1, topic: defaultTopic });
        onClose();
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#1e1e2e]/80">
            <div className="bg-[#313244] border border-[#45475a] rounded-lg shadow-lg p-8 min-w-[320px] max-w-full">
                <h2 className="text-xl font-bold mb-4 text-[#cdd6f4]">Quiz: {defaultTopic || "General Knowledge"}</h2>
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="flex justify-end space-x-2">
                        <Button type="button" onClick={onClose} className="bg-[#f38ba8] text-[#1e1e2e]">Cancel</Button>
                        <Button type="submit" className="bg-[#cba6f7] text-[#1e1e2e]" disabled={submitted}>Submit</Button>
                    </div>
                </form>
            </div>
        </div>
...
// If QuizModal is imported from another file, ensure that file exports QuizModal with the correct props.
...
export default function ChatPage() {
    ...
    async function handleReadAloud(text: string) {
        try {
            ...
            const audio = new window.Audio(`data:audio/mp3;base64,${data.audio}`)
            audio.play()
        } catch {
