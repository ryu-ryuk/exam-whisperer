...
async def generate_quiz(topic: str, difficulty: str = "medium") -> dict:
    """Generate a 3-question MCQ quiz with explanations."""
    if not topic.strip():
    ...
    try:
        ...
        logging.info(f"Generated quiz for topic: {topic}, difficulty: {difficulty}")
        return quiz_data
...
async def evaluate_quiz_answer(user_id: str, topic: str, question_index: int, user_answer: int, quiz_data: dict) -> dict:
    ...
    feedback_prompt = (
        ...
        f"User's answer: {chr(97 + user_answer)} ({question['options'][user_answer]})\n"
        f"Correct answer: {chr(97 + correct_answer)} ({question['options'][correct_answer]})\n"
    ...
    try:
        ...
        return {
            ...
            "correct_answer": {
                ...
                "text": question["options"][correct_answer]
            },
            "user_answer": {
                "index": user_answer,
                "text": question["options"][user_answer]
            }
        }
    except LLMProviderError as e:
