...
def parse_quiz_text(raw: str) -> dict:
    """Convert LLM's plain text quiz into structured dict. Ensures 'options' only contains answer choices, and 'explanations' only explanations."""
    questions = []
    current_question = {}
    ...
    while i < len(lines):
        ...
        if line.startswith("Question:"):
            ...
            # Check for explanation in the next line
            if i + 1 < len(lines) and lines[i + 1].strip().startswith(f"- {option_key}:"):
                # If next line is another option, no explanation for this option
                current_explanations[option_key] = ""
            elif i + 1 < len(lines) and not lines[i + 1].startswith("-") and not lines[i + 1].startswith("Answer:"):
    ...
    # Ensure only the first 4 options are kept (a-d)
    for q in questions:
        if len(q["options"]) > 4:
            q["options"] = q["options"][:4]
        # Also trim explanations to a-d
        q["explanations"] = {k: v for k, v in q["explanations"].items() if k in ["a", "b", "c", "d"]}
